...
$Tagstable = mysqli_query($conn, $custQuery);
$row = mysqli_fetch_array($Tagstable);
$cname = $row['Username'];
...
<style>
    ...
    #popover-content {
        display: none;
        max-height: 300px;
        /* Set a maximum height for the notifications div */
        overflow-y: auto;
...
<body>
    ...
    <div class="navbar d-flex justify-content-center bg-white">
        <!-- Nav logo -->
        <div class="navlogo" style="display: flex;">
            <p>VENT </p>
            <img id="navlogoimg" src="../assests/images/image.png" alt="Quora logo" height="50px" class="mr-3">
        ...
        <!-- Notifications Icon -->
        <div id="Notifications" class="navicon grayicon" title="Notifications" data-bs-container="body"
        ...
        <div class="navsearch" style="display:flex;">
            ...
            <input id="navinput" type="text" placeholder="Search Vent">
    ...
    <div id="profiledropdown" class="flex-column align-items-center">
        ...
        <div class="d-flex flex-column card p-2">
            <div class="pointer d-flex pt-2 bg-white" onclick="window.location.href='profile.php'">
                ...
                <div class="col-md-8">
                    <span style="font-size: larger; font-weight:bolder; position:relative; top:10%; left: -15%">
                        <?php echo $cname; ?>
                    </span>
                </div>
            </div>
    ...
    <!-- Pop-up when Add question is Pressed -->
    <form action="../connection/insert-question.php" class="submit_question" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="qcard" id="qcard">
                        ...
                        <div class="qcardbody">
                            ...
                            <div class="qcardques d-flex">
                                <textarea placeholder="Start your question with What, Why, How etc." id="quesfield"
                                    name="quesfield"></textarea>
                            </div>
                        ...
                        <div class="qcardfooter d-flex justify-content-between">
                            <div>
                                <button type="button" class="btn bg-light text-muted" id="quesphotobtn">Add
                                    photo</button>
                                <span class="text-muted" id="qname-msg" style="font-size: x-small;"></span>
                            </div>
                            <div class="d-flex justify-content-end">
                                <button id="qcancel" onclick="hideques()">cancel</button>
                                <button type="button" id="qnext" name="qnext" class="btn btn-primary"
                                    onclick="showCat()">Add Question</button>
                            </div>
                        </div>
            ...
            <div id="question-cat-container">
                <div class="qcardcontainer">
                    <div class="qcard" id="catcard">
                        ...
                        <div class="qcardheader d-flex">
                            <div id="qback" class="col-md-1"
                                onclick="document.getElementById('question-categories').style.display = 'none'">
                        ...
                        <div class="qcardbody">
                            <div class="qcardinfo d-flex">
                                <img id="qcardavatar" name="qcardavatar" src="../assets/images/dp3.jpg">
                                <span>
                                    <?php $cname ?>
                                </span>
                                <span>
                                    <?php "../assests/images/image.jpeg" ?>
                                </span>
                            </div>
                            <div class="qcardques d-flex flex-column">
                                <span class="py-2">Choose the Tags of your question for better and quicker
                                    answers</span>
                                ...
                                if ($Tagstable = mysqli_query($conne, $TagsQuery)) {
                                    if (mysqli_num_rows($Tagstable) > 0) {
                                        while ($row = mysqli_fetch_array($Tagstable)) {
                                            $tagName = $row['Tagname'];
                                            echo
                                                "<div class='form-check form-switch py-1'>
                                                    <input class='form-check-input' type='checkbox' id='$tagName' name='cat_check[]' value='$tagName'>
    ...
    <!-- End of Add Questions Pop over -->

    <!-- Hidden Notifications Content -->
    <div id="popover-content" style="display: none;">
        <?php

        $Uid = $_SESSION['UserId'];

        // Check if the current user has asked any questions and if anyone has anwsered his questions.
        $checkQuestionsQuery = "SELECT * FROM questions WHERE UserId = '$Uid'";
        $result = mysqli_query($conn, $checkQuestionsQuery);
        while ($row = mysqli_fetch_array($result)) {
            $questionId = $row['questionId'];

            // Check if there are answers by other users for the current user's questions
            $checkAnswersQuery = "SELECT * FROM answers WHERE questionId = '$questionId' AND UserId != '$Uid'";
            $answersResult = mysqli_query($conn, $checkAnswersQuery);

            while ($answerRow = mysqli_fetch_array($answersResult)) {
                $answeringUserId = $answerRow['UserId'];

                // Fetch user information for the notification
                $getUserInfoQuery = "SELECT * FROM userprofile WHERE UserId = '$answeringUserId'";
                $userInfoResult = mysqli_query($conn, $getUserInfoQuery);


                if ($userInfo = mysqli_fetch_array($userInfoResult)) {
                    $answeringUserName = $userInfo['Username'];

                    // Display or store the notification information as needed
                    echo "<div><h6> $answeringUserName<span> has answered your question<span><h6>
                        <hr></div>
                        ";
                } else {
                    echo "<div>No One Anwsered Yet!</div>";
                }
            }
        }


        // to check if any of the anwsered given by user has recieved anyvotes for not.
        $getAnswersQuery = "SELECT * FROM answers WHERE UserId = '$Uid'";
        $answersResult = mysqli_query($conn, $getAnswersQuery);

        while ($answerRow = mysqli_fetch_array($answersResult)) {
            $questionId = $answerRow['questionId'];
            $upvotes = $answerRow['UpVotes'];
            $downvotes = $answerRow['DownVotes'];
            ...
            if ($questionInfo = mysqli_fetch_array($questionResult)) {
                $askerUserId = $questionInfo['UserId'];
                $askerUsername = ""; // Fetch asker's username based on UserId
                $askedQuestion = $questionInfo['body'];

                // Display the notification information 
                if ($upvotes != Null) {
                    echo "<div><p>Your answer to the question \"$askedQuestion\" asked by $askerUsername received $upvotes upvotes.<p><hr></div>";
                }
                if ($downvotes != Null) {
                    echo "<div><p>Your answer to the question \"$askedQuestion\" asked by $askerUsername received $downvotes downvotes.<p><hr></div>";
                }
            }
        }
    ...
    <!-- End of hidden Notification Content -->


    <!--Imported Java Scripts -->
    ...
    <!--Imported Java Scripts  -->
    <!-- To Make navicons Active and InActive -->
    <script>
...
<!-- For Adding Questions  -->
    <script>
        ...
        document.getElementById("answer").onclick = function () {
            window.location.href = "anwsers.php";
        };
        ...
        $(document).ready(function () {
            ...
            $('#Notifications').click(function () {
                // Use the preloaded content in the popover
                $('#Notifications').attr('data-bs-content', $('#popover-content').html());
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#navinput').on('input', function () {
                ...
                $.ajax({
                    type: 'POST', // You can change the HTTP method based on your server-side implementation
                    url: '../connection/search.php', // Replace with the actual endpoint to handle the search
