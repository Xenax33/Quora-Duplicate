...
// Question table data
$quesQuery = "SELECT * FROM questions WHERE questionId= '$qid'";
...
// Customer table data
$qcid = $question['UserId'];
$qcustquery = "SELECT * FROM userprofile WHERE UserId= '$qcid'";
$asker = mysqli_query($conn, $qcustquery);

if (!$asker) {
    die("Asker query failed: " . mysqli_error($conn));
}
        ...
        <div class="d-flex flex-column w-100">
            ...
            <div class="jumbotron w-100">
                ...
                <small class="text-muted">Tags:
                    <?php echo $question['tags']; ?>
                </small>
            </div>

            <div class="card p-3">
                <span>Post your answer here</span>
                <form method="post" action="../connection/submitanwser.php">
                    <textarea name="answerfield" id="answerfield" rows="5" class="w-100 my-2 p-2"
                        placeholder="Please explain your answer in detail..."></textarea>
            ...
            <?php
            if ($answers && mysqli_num_rows($answers) > 0) {
                while ($ans = mysqli_fetch_array($answers)) {

                    $a_cid = $ans['UserId'];
                    $ans_body = $ans['body'];
                    $ans_date = $ans['CreatedAt'];

                    $ansCustQuery = "SELECT * FROM userprofile WHERE UserId = '$a_cid'";
                    $ansCust = mysqli_query($conn, $ansCustQuery);

                    if (!$ansCust) {
                        die("Answer customer query failed: " . mysqli_error($conn));
                    }

                    $ansCust = mysqli_fetch_array($ansCust);

                    $acustname = $ansCust['Username'];
                    $acustdp = $ansCust['Bio'];
